...
Sometimes it can seem like a thin line between writing good distractor options and yet "Avoid Trick Questions". To help with this, just try to write basic straightforward questions with one distinctly correct answer and several distinctly incorrect ones.
...
Try to avoid testing on specific patterns or architectural decisions which can be viewed as highly subjective or opinionated and presenting them as fact. If there is a pattern that you'd like to ask about, it might **not** be best to phrase the question to suggest the pattern is "better than" or "worse than" other options -- especially if these concepts are at all debated in the respective field. As an example, if you were to ask a question about the *Singleton Design Pattern*, you wouldn't want to ask something like "Which pattern is better for...", where "Singleton" is your answer. While you might be technically correct, "better for" is a highly subjective phrase. Instead, phrase the question in a very fact-based objective way so that anyone who does understand Singleton, would know what the obvious answer is. In this case, perhaps a question as to how Singletons technically work might be better.
